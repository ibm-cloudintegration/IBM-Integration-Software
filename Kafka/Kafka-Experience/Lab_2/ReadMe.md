...
![alt text](./images/kafka-lab2-component-diagram.png)
...
3\. Now enter the name of your topic.  Since this is a shared Kafka broker, use your userid as part of the topic name.  In this example, we are logged in as student1 so the topic name is **student1.toolkit-topic**.  Click **Next**.
...
7\. Now you will be back on the Topics screen.  You may see other users topics in the list but make sure you see your topic.  In this example, we just created the ex: **student1.toolkit-topic** topic.
...
Click on the KafkaProducer in the flow make sure you click on the Properties tab.   This is where you will make your updates to this flow.   For the Topic name you will replace the XXXX with your user id.  In this example it would be student1. 
...
9\. Now we will open the Event Stream Policy where we will configure the connection info to the Kafka broker.  Copy and paste the Event Streams Kafka Cluster bootstrap URL you captured in the pre-req lab.<br>
...
8\. You will now see the project in the Application Development on the left sdie.   
   ...
   - <span style="color:red"><b>(4)</b></span> This is where you will make your updates to this flow.   For the Topic name you will replace the XXXX with your user id.  In this example it would be student1.
...
So for this I would use 1 for student1 user.
If you are say cstudent10 you would change the X to 10. 
...
6\. In the next window enter the name of the configuration. In my case I used student1-esdemo-scram-credential. Followed by a brief description i.e. Credential to connect to es-demo using SCRAM. Finally use the following information as a reference to enter the data in the text box.
 
Resource Name | User | Password
--------------|------|---------
***truststore::truststorePass*** | dummy | &lt;Password obtained when downloading the PKCS12 certificate">
***kafka::aceflowsSecId*** | &lt;This will be your user id i.e. student1 used to create SCRAM credentials.> | &lt;Password obtained for your user id when you created your SCRAM credentials to connect to Event Streams">  

Here is an example from when the lab was created:

```
truststore::truststorePass dummy CMOzxxfSYGMT
kafka::aceflowsSecId student1 hAUDJTO76Fx0
```
...
Enter the name of the policy i.e. **student1-mq-policy** , add a brief description if you want to, i.e. **Policy to connect to student1 QMgr** and finally click on hyper link **Drag and drop a single file here or click to import** to import the zip file we created in the previous section. 
...
2\.From the Dashboard screen click on the Deploy Integrations + button. 
...
<!--
to the Runtime container section and change the CPU request and Memory request.  This will help to speed up the deployment for the lab.   Then click **Create** 
-->
...
11\. Now let's go to the Red Hat Openshift Console and make sure you sign in with your same userid and password.  Select Workloads and select Pods to see all your running pods in your Project (Namespace).   In this example it is student1.
